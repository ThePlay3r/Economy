...
public class EconomyCommand extends CommandUtil implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0){
            ...
            sendMessage(player, CfgLang.lang.get(Lang.ECONOMY_SUCCESS).replace("%money", VaultUtil.getBalance(player)+""));
            return true;
        }
            ...
            if (args[0].equalsIgnoreCase("balance")){
                ...
                sendMessage(sender, CfgLang.lang.get(Lang.ECONOMY_BALANCE_SUCCESS).replace("%player", args[1]).replace("%money", VaultUtil.getBalance(args[1])+""));
                return true;
            }
        }
            ...
            if (args[0].equalsIgnoreCase("send")){
                ...
                if (amount > playerBalance){
                    sendMessage(player, CfgLang.lang.get(Lang.ECONOMY_SEND_FAILURE_TOO_MUCH));
                    return false;
                }
                VaultUtil.deposit(args[1], amount);
                VaultUtil.withdraw(player, amount);
                sendMessage(player, CfgLang.lang.get(Lang.ECONOMY_SEND_SUCCESS).replace("%money", amount+"").replace("%player", args[1]));
                if (PlayerUtil.isPlayer(args[1])){
                    Player requestedPlayer = Bukkit.getPlayer(args[1]);
                    sendMessage(requestedPlayer, CfgLang.lang.get(Lang.ECONOMY_SEND_SUCCESS_NOTIFY).replace("%money", amount+"").replace("%player", player.getName()));
                }
                return true;
            }
        }
