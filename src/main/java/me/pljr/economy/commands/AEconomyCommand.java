    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1){
            ...
            if (args[0].equalsIgnoreCase("set")){
                ...
                sender.sendMessage(CfgLang.lang.get(Lang.AECONOMY_SET_SUCCESS).replace("%player", args[1]).replace("%money", args[2]));
                if (PlayerUtil.isPlayer(args[1])){
                    Player player = Bukkit.getPlayer(args[1]);
                    player.sendMessage(CfgLang.lang.get(Lang.AECONOMY_SET_SUCCESS_NOTIFY).replace("%player", sender.getName()));
                }
                return true;
            }
            ...
            if (args[0].equalsIgnoreCase("add")){
                ...
                VaultUtil.deposit(args[1], amount);
                sender.sendMessage(CfgLang.lang.get(Lang.AECONOMY_ADD_SUCCESS).replace("%player", args[1]).replace("%money", args[2]));
                if (PlayerUtil.isPlayer(args[1])){
                    Player player = Bukkit.getPlayer(args[1]);
                    player.sendMessage(CfgLang.lang.get(Lang.AECONOMY_ADD_SUCCESS_NOTIFY).replace("%player", sender.getName()));
                }
                return true;
            }
            ...
            if (args[0].equalsIgnoreCase("remove")){
                ...
                if ((playerMoney-amount)<0){
                    sender.sendMessage(CfgLang.lang.get(Lang.AECONOMY_REMOVE_FAILURE_TOO_MUCH).replace("%player", args[1]));
                    return false;
                }
                VaultUtil.withdraw(args[1], amount);
                sender.sendMessage(CfgLang.lang.get(Lang.AECONOMY_REMOVE_SUCCESS).replace("%player", args[1]).replace("%money", args[2]));
                if (PlayerUtil.isPlayer(args[1])){
                    Player player = Bukkit.getPlayer(args[1]);
                    player.sendMessage(CfgLang.lang.get(Lang.AECONOMY_REMOVE_SUCCESS_NOTIFY).replace("%player", sender.getName()));
                }
                return true;
            }
        }
